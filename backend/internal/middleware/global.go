...
func (global *GlobalMiddlewares) GlobalErrorHanlder(err error, c echo.Context) {
	...
	// Only do this for errors that haven't already been converted to HTTPERrror
	var httpErr *errs.HTTPError
	...
	// New process the possibly converted error
	var echoErr *echo.HTTPError
	var status int
	var code string
	var message string
	...
	// User enhanced logger from context which already includes request_id, method, path, ip, user context, and trace context
	logger := *GetLogger(c)
