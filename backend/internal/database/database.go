...
import (
	...
	"github.com/gopeshwark/go-boilerplate/internal/config"
	loggerConfig "github.com/gopeshwark/go-boilerplate/internal/logger"
...
// TrraceQueryStart implements pgx tracer interface
...
func New(cfg *config.Config, logger *zerolog.Logger, loggerService *loggerConfig.LoggerService) (*Database, error) {
	...
	dsn := fmt.Sprintf("postgres://%s:%s@%s/%s?sslmode=%s", cfg.Database.User, encodedPassword, hostPort, cfg.Dtabase.Name, cfg.Database.SSLMode)
	...
	if cfg.Primary.Env == "local" {
		...
		// Chanintracers - New RElic first,, then local logging
