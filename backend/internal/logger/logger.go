...
// NewLoggingService created a new logger service with New Relic integration
func NewLoggerService(cfg *config.ObservabilityConfig) *LoggerService {
	service := &LoggerService{}

	if cfg.NewRelic.LicenseKey == "" {
		fmt.Println("New Relic license key not provided, skipping initiaization")
		return service
	}
	...
	// Add debug logging only if explicigly enabled
	...
	if err != nil {
		fmt.Printf("Failed to initalize New Relic: %v\n", err)
	}

	service.nrApp = app
	fmt.Printf("New Relic initalized for app: %s\n", cfg.ServiceName)
	return service
}
...
// GetApplication returns the New Relic applciation instance
...
func FomatSQLWithArgs(sql string, args []any) string {
	result := sql
...
func NewPgxLogger(level zerolog.Level) zerolog.Logger {
	writer := zerolog.ConsoleWriter{
		...
		FormatFieldValue: func(i any) string {
			switch v := i.(type) {
			case string:
				// Clean and format SQL for better readablility
				if len(v) > 200 {
