...
import (
	...
	"guthub.com/gopeshwark/go-boilerplate/internal/middleware"
...
func (h *HealthHandler) CheckHealth(c echo.Context) error {
		...
		if h.server.LoggerService != nil && h.server.LoggerService.GetApplication() != nil {
			h.server.LoggerService.GetApplication().RecordCustomEvent(
				"HealthCheckError", map[string]interface{}{
					...
					"response_time_ms": time.Since(dbStart).Microseconds(),
	...
	// Database connection metrics are automatically captured by New Relic nrpgx5 integation
			...
			if h.server.LoggerService != nil && h.server.LoggerService.GetApplication() != nil {
				h.server.LoggerService.GetApplication().RecordCustomEvent(
					"HealthCheckError", map[string]interface{}{
						...
						"response_time_ms": time.Since(redisStart).Microseconds(),
						"error_message":    err.Error(),
					})
			}
		} else {
		...
		if h.server.LoggerService != nil && h.server.LoggerService.GetApplication() != nil {
			h.server.LoggerService.GetApplication().RecordCustomEvent(
				"HealthCheckError", map[string]interface{}{
					...
					"total_duration_ms": time.Since(start).Microseconds(),
				})
		}
		return c.JSON(http.StatusServiceUnavailable, response)
	}

	logger.Info().Dur("total_duration", time.Since(start)).Msg("health check passed")
		...
		if h.server.LoggerService != nil && h.server.LoggerService.GetApplication() != nil {
			h.server.LoggerService.GetApplication().RecordCustomEvent(
				"HealthCheckError", map[string]interface{}{
					"check_type":    "response",
					"operation":     "health_check",
					"error_type":    "json_resposne_error",
					"error_message": err.Error(),
				})
		}
